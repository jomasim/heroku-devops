...
class SalesController(Resource):
    ...
    def get(self, sale_id=None):
        if self.get_all_sales(sale_id):
            return make_response(jsonify({'sales': self.get_all_sales(sale_id)}), 200)
        else:
    ...
    def get_validation_errors(self, data):
        ...
        if all_errors == None:
            return self.get_line_item_errors(data)

        return all_errors

    def get_line_item_errors(self, data):
            all_errors = {}

            item_count = int(data['line_items']['item_count'])
            selling_price = int(data['line_items']['selling_price'])
